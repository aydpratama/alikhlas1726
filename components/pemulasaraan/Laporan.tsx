...
export function ReportView({ year, members, duesData }: ReportViewProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    "summary" | "Tetap" | "Tetap Tambahan" | "Umum"
  >("summary");
  ...
  const { groupedData, grandTotals } = useMemo(() => {
    ...
    members.forEach((m) => {
      ...
      data[key].grandTotal =
        data[key].totalUmum + data[key].totalTetap + data[key].totalTambahan;
    });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col xl:flex-row justify-between xl:items-end gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 text-sm max-w-md leading-relaxed">
            Rekapitulasi lengkap data iuran pemulasaraan per RT/RW. Gunakan
            filter di sebelah kanan untuk mempersempit jangkauan data.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <CardStat
          title="Total Anggota"
          value={(
            grandTotals.countTetap +
            grandTotals.countTambahan +
            grandTotals.countUmum
          ).toString()}
        ...
        <div className="overflow-x-auto max-h-[700px] overflow-y-auto relative custom-scrollbar">
          <table className="w-full text-xs text-left whitespace-nowrap border-collapse">
            <thead className="bg-slate-50 text-slate-600 font-bold sticky top-0 z-20 shadow-sm uppercase tracking-wider text-[10px]">
              {activeTab === "summary" ? (
                <tr>
                  <th className="px-4 py-4 text-center sticky left-0 bg-slate-50 z-30 border-b border-slate-200">
                    RW
                  </th>
                  <th className="px-4 py-4 text-center sticky left-[calc(3rem)] bg-slate-50 z-30 border-b border-r border-slate-200">
                    RT
                  </th>
                  <th className="px-6 py-4 text-right border-b border-slate-200">
                    Pend. Umum
                  </th>
                  <th className="px-6 py-4 text-right border-b border-slate-200">
                    Iuran Bulanan
                  ...
                  <th className="px-6 py-4 text-right bg-emerald-50 text-emerald-800 border-b border-emerald-100 sticky right-0 z-30 shadow-l-sm">
                    GRAND TOTAL
                  </th>
                </tr>
              ) : activeTab === "Umum" ? (
                <tr>
                  ...
                  {MONTHS.map((m) => (
                    <th
                      key={m.key}
                      className="px-3 py-4 text-right border-b border-slate-200"
                    >
                      {m.label}
                    </th>
                  ))}
                ...
                <tr>
                  <th className="px-4 py-4 text-center sticky left-0 bg-slate-50 z-30 border-b border-slate-200 w-20">
                    RW
                  </th>
                  <th className="px-4 py-4 text-center sticky left-20 bg-slate-50 z-30 border-b border-r border-slate-200 w-20">
                    RT
                  </th>
                  <th className="px-6 py-4 text-right border-b border-slate-200">
                    Jumlah Anggota
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {groupedData.length === 0 ? (
                <tr>
                  <td
                    colSpan={20}
                    className="px-10 py-16 text-center text-slate-400"
                  >
                ...
                groupedData.map((row, idx) => (
                  ...
                  >
                    {activeTab === "summary" ? (
                      <>
                        <td className="px-4 py-3 text-center font-bold text-slate-700 sticky left-0 bg-white group-hover:bg-slate-50 z-10">
                          {row.rw}
                        </td>
                        <td className="px-4 py-3 text-center font-bold text-slate-700 sticky left-[calc(3rem)] bg-white group-hover:bg-slate-50 z-10 border-r border-slate-200 group-hover:border-slate-200">
                          {row.rt}
                        </td>
                        <td className="px-6 py-3 text-right text-slate-600 tabular-nums">
                          {formatCurrency(row.pendaftaranUmum)}
                        </td>
                        <td className="px-6 py-3 text-right text-emerald-600 font-medium tabular-nums bg-emerald-50/30">
                          {formatCurrency(
                            row.monthlyUmum.reduce((a, b) => a + b, 0),
                          )}
                        </td>
                        <td className="px-6 py-3 text-right text-slate-600 tabular-nums">
                          {formatCurrency(row.totalTetap)}
                        </td>
                        <td className="px-6 py-3 text-right text-slate-600 tabular-nums">
                          {formatCurrency(row.totalTambahan)}
                        </td>
                      ...
                      <>
                        ...
                        {row.monthlyUmum.map((val, midx) => (
                          <td
                            key={midx}
                            className={`px-3 py-3 text-right tabular-nums ${val > 0 ? "text-emerald-600 font-medium bg-emerald-50/30" : "text-slate-300"}`}
                          >
                            {formatCurrency(val)}
                          </td>
                        ))}
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 text-center font-bold text-slate-700 sticky left-20 bg-white group-hover:bg-slate-50 z-10 border-r border-slate-200 group-hover:border-slate-200">
                          {row.rt}
                        </td>
                ...
                <>
                  {activeTab === "summary" ? (
                    <tr>
                      <td
                        colSpan={2}
                        className="px-4 py-4 text-center sticky left-0 bg-slate-900 z-30 border-r border-slate-800 text-[10px] uppercase tracking-wider"
                      >
                        TOTAL SELURUH
                      </td>
                      <td className="px-6 py-4 text-right tabular-nums">
                        {formatCurrency(grandTotals.pendaftaranUmum)}
                      </td>
                      <td className="px-6 py-4 text-right text-emerald-300 tabular-nums">
                        {formatCurrency(
                          grandTotals.monthlyUmum.reduce((a, b) => a + b, 0),
                        )}
                      </td>
                      <td className="px-6 py-4 text-right tabular-nums">
                        {formatCurrency(grandTotals.totalTetap)}
                      </td>
                      <td className="px-6 py-4 text-right tabular-nums">
                        {formatCurrency(grandTotals.totalTambahan)}
                      </td>
                    ...
                    <tr>
                      ...
                      {grandTotals.monthlyUmum.map((val, idx) => (
                        <td
                          key={idx}
                          className="px-3 py-4 text-right text-emerald-300 tabular-nums"
                        >
                          {formatCurrency(val)}
                        </td>
                      ))}
                      <td className="px-6 py-4 text-right bg-emerald-600 sticky right-0 z-30 shadow-l-md tabular-nums text-sm">
                        {formatCurrency(grandTotals.totalUmum)}
                      </td>
                    </tr>
                  ) : (
                    <tr>
                      ...
                      <td className="px-6 py-4 text-right bg-emerald-600 sticky right-0 z-30 shadow-l-md tabular-nums text-sm">
                        {formatCurrency(
                          activeTab === "Tetap"
                            ? grandTotals.totalTetap
                            : grandTotals.totalTambahan,
                        )}
                      </td>
                    </tr>
                  )}
      ...
      <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            Laporan ini digenerate secara otomatis dari data transaksi harian.
            Pastikan seluruh input data anggota dan pembayaran telah
            diverifikasi sebelum mencetak laporan ini untuk kebutuhan rapat atau
            audit. Nominal ditampilkan dalam mata uang Rupiah penuh.
          </p>
        </div>
      </div>
    </div>
...
function CardStat({
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow relative overflow-hidden group">
      <div
        className={`absolute top-0 right-0 w-16 h-16 opacity-10 rounded-bl-full transition-transform group-hover:scale-110 ${colorClasses[color as keyof typeof colorClasses].replace("bg-", "bg-")}`}
      ></div>
      <div className="flex items-start justify-between mb-4">
        ...
        <div
          className={`p-2 rounded-xl ${colorClasses[color as keyof typeof colorClasses]}`}
        >
          {icon}
        </div>
      </div>
      <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500">
        <span
          className={`inline-block w-1.5 h-1.5 rounded-full ${color === "emerald" ? "bg-emerald-500" : color === "blue" ? "bg-blue-500" : color === "amber" ? "bg-amber-500" : "bg-purple-500"}`}
        ></span>
        {trend}
      </div>
    </div>
