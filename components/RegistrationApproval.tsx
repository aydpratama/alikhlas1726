...
export function RegistrationApproval({ isAdminProp, isAdminLoadingProp, onDataChange }: RegistrationApprovalProps) {
    ...
    const handleApprove = async (members: any[]) => {
        const familyName = members[0].nama_kepala_keluarga || members[0].nama_lengkap;
        if (!confirm(`Setujui pendaftaran untuk keluarga ${familyName} (${members.length} orang)?`)) return;
        ...
        try {
            ...
            const yearMembers = latestMembers?.filter(m => m.no_anggota?.endsWith(`/PEM/${yearStr}`)) || [];
            
            if (yearMembers.length > 0) {
                const numbers = yearMembers.map(m => parseInt(m.no_anggota.split('/')[0])).filter(n => !isNaN(n));
                if (numbers.length > 0) {
            ...
            for (let i = 0; i < members.length; i++) {
                const reg = members[i];
                const newNoAnggota = `${nextNum}/PEM/${yearStr}.${String(i + 1).padStart(2, '0')}`;
    ...
    const groupedRegistrations = registrations.reduce((acc: { [key: string]: any[] }, reg: any) => {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="font-bold text-lg text-slate-900">Menunggu Persetujuan</h3>
                <Button variant="ghost" size="sm" onClick={fetchRegistrations} disabled={isLoading} className="h-10 sm:h-8 text-slate-500 hover:text-emerald-600">
                    Refresh
                </Button>
            </div>
            ...
            {registrations.length === 0 ? (
                <div className="p-12 text-center text-slate-400 border border-slate-200 rounded-2xl bg-slate-50/50 border-dashed">
                    {isLoading ? (
                        <div className="flex flex-col items-center gap-2">
                            ...
                            <span className="text-xs font-medium">Memuat data pendaftaran...</span>
                        </div>
                    ) : (
                ...
                <div className="grid gap-6">
                    {Object.entries(groupedRegistrations).map(([kepalaKeluarga, members]: [string, any[]]) => (
                        <div key={kepalaKeluarga} className="border border-slate-200 rounded-2xl bg-white shadow-sm overflow-hidden transition-all hover:border-slate-200">
                            ...
                            <div className="bg-slate-50/80 px-4 py-3 border-b border-slate-200 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Kepala Keluarga</p>
                                        <h4 className="font-bold text-slate-900 leading-none">{kepalaKeluarga}</h4>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">Alamat</p>
                                    <p className="text-xs font-medium text-slate-600 leading-none">
                                        RT {members[0].rt}/RW {members[0].rw} • {members[0].alamat}
                                    </p>
                                </div>
                            </div>
                                    ...
                                    <div key={reg.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <p className="font-bold text-slate-800">{reg.nama_lengkap}</p>
                                                <Badge variant="outline" className="text-[9px] uppercase h-4 px-1 border-slate-200 text-slate-500">{reg.hubungan_keluarga}</Badge>
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <p className="text-[10px] text-slate-500 font-medium">
                                                    Kontak: <span className="text-slate-700">{reg.no_telepon || '-'}</span>
                                                </p>
                                                <div className="w-1 h-1 rounded-full bg-slate-200"></div>
                                                <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 border-none text-[9px] h-4 px-1.5 font-bold">
                                                    {reg.jenis_anggota}
                                                </Badge>
                                                <div className="w-1 h-1 rounded-full bg-slate-200"></div>
